  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      try {
        ...
        const res = await fetch(`${env.server}/legacy/game/profile/${gameId}`, options);
        ...
        <div className="relative w-full bg-gray-200 rounded-xl">
          {info?.coverPhoto ? (
            <div className="overflow-hidden rounded-b-xl">
              <Parallax bgImage={info.coverPhoto} bgImageAlt="Game Cover" blur={2}>
            ...
            <div className="aspect-[16/4] bg-[#f7b43d] flex items-center justify-center">
              <h1 className="text-3xl font-bold text-white">{info?.name || 'Unknown Game'}</h1>
            </div>
          )}
        </div>
